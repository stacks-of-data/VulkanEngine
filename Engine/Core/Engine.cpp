...
LayersCheckResult checkLayersSupport(std::vector<const char *> &layers) {
  ...
  if (res != VK_SUCCESS)
    throw EngineExceptions::VKCallFailure("vkEnumerateInstanceLayerProperties",
                                          res);
...
std::vector<const char *> getVulkanExtensions() {
  ...
  std::vector<const char *> extensions(extensionsArr,
                                       extensionsArr + extensionsCount);
...
void Engine::selectPhysicalDevice() {
  ...
  res = vkEnumeratePhysicalDevices(this->m_instance, &devicesCount,
                                   devices.data());
  if (res != VK_SUCCESS)
    EngineExceptions::VKCallFailure("vkEnumeratePhysicalDevices", res);
...
Engine::Engine()
    : m_glfwInitialized(false), m_vkInitialized(false), m_window(nullptr),
      m_instance(NULL) {
  if (DEBUG) {
...
EngineExceptions::VKCallFailure::VKCallFailure(const char *funcName,
                                               VkResult errCode) {
